#!/usr/bin/env python3
"""
Factor analysis for long-context modality comparison:
How do paper features correlate with model accuracy for different input orderings?
...
def plot_factor_analysis(all_correlations: dict, output_dir: Path):
    """Create 2x3 grid: 2 orderings x 3 factor groups."""
...
def plot_prediction_r2(all_correlations: dict, output_dir: Path):
    """Create 2x3 grid showing R² between factors and binary prediction."""
    ...
    for row_idx, variant in enumerate(variants):
        ...
        for col_idx, (factor_labels, col_title) in enumerate([
            ...
            # Add significance markers and ratio labels
            for i, f in enumerate(factors):
                label_parts = []
                if correlations[f]['pred_p'] < 0.05:
                    label_parts.append('*')

                # For Vision+Text model, show ratio vs Text+Vision
                if variant == "Vision + Text":
                    vision_text_r2 = correlations[f]['pred_r2']
                    text_vision_r2 = all_correlations.get("Text + Vision", {}).get(f, {}).get('pred_r2', 0)

                    if text_vision_r2 > 0.001:
                        ratio = vision_text_r2 / text_vision_r2
                        label_parts.append(f'[{ratio:.1f}x vs T+V]')
...
def plot_prediction_ratio(all_correlations: dict, output_dir: Path):
    """Create 2x3 grid showing log ratio of prediction R² vs ground truth R²."""
