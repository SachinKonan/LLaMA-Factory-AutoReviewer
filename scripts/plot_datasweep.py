...
Usage:
    ...
    python scripts/plot_datasweep.py --runs balanced,balanced_deepreview,original
"""
...
def plot_data_sweep_losses(ax, saves_dir: Path, runs_filter: list[str] | None = None, max_y_val: float | None = None):
    """Plot training loss curves for all dataset variants."""
    configs = discover_configs(saves_dir)

    # Filter configs if --runs specified
    if runs_filter:
        configs = {k: v for k, v in configs.items() if k in runs_filter}
...
def main():
    parser = argparse.ArgumentParser(description="Plot training loss curves for data sweep experiments.")
    parser.add_argument(
        "--runs",
        type=str,
        default=None,
        help="Comma-separated list of runs to plot (e.g., balanced,balanced_deepreview,original)"
    )
    parser.add_argument(
        "--max-y-val",
        type=float,
    ...
    plot_data_sweep_losses(ax, SAVES_DIR, runs_filter=runs_filter, max_y_val=args.max_y_val)
