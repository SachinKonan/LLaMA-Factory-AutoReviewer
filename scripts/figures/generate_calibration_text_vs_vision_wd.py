#!/usr/bin/env python3
"""
Calibration plot: text vs vision (optim_search_2026, with weight decay).
Side-by-side: (a) per-bin reliability, (b) cumulative accuracy.
Best checkpoints:
  text (wd):   ckpt-2644 (66.7%)
  vision (wd): ckpt-5296 (67.5%)
"""
...
def main():
    ...
    print(f"Text (wd):   {text_corr.mean():.4f} ({len(text_corr)} samples)")
    print(f"Vision (wd): {vis_corr.mean():.4f} ({len(vis_corr)} samples)")

    fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(18, 7))

    legend_handles = [
        Line2D([0], [0], color=BLUE, marker="o", linewidth=LINEWIDTH,
               markersize=MARKERSIZE, label="Text w/ WD (acc.)"),
        Line2D([0], [0], color=ORANGE, marker="o", linewidth=LINEWIDTH,
               markersize=MARKERSIZE, label="Vision w/ WD (acc.)"),
    ...
    out = OUTPUT_DIR / "calibration_text_vs_vision_wd"
