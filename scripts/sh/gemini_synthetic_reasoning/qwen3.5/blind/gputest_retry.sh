#!/bin/bash
# Retry failed blind text reviews — Qwen3.5-122B-A10B — gpu-test partition
# Papers that got 0/3 good completions across all waves.
# Key change: --max-model-len 28672 (up from 20480) to handle longer papers.
# Usage: sbatch scripts/sh/gemini_synthetic_reasoning/qwen3.5/blind/gputest_retry.sh

#SBATCH --job-name=qwen-blind-retry-v2
#SBATCH --array=0-7
...
echo "=== Qwen Blind Text Review RETRY v2 (gpu-test) ==="
...
nohup vllm serve "$MODEL_PATH" \
    -dp 4 --enable-expert-parallel --language-model-only \
    --reasoning-parser qwen3 --enable-prefix-caching \
    --max-model-len 28672 --max-num-batched-tokens 4096 \
    > "logs/qwen_review/${MODE}/vllm_retry_v2_shard${SLURM_ARRAY_TASK_ID}.log" 2>&1 &
VLLM_PID=$!
...
python scripts/qwen_batch_review.py \
    ...
    --workers $WORKERS --num_shards 8 \
    --retry_file results/reviews/blind_failed_sids_v2.txt \
    --shard_prefix retry_v2 --wait_for_server 900 \
