#!/bin/bash
# Retry failed blind text reviews — Qwen3.5-122B-A10B — ailab partition
# Papers that got 0/3 good completions in the initial run.
# Key change: --max-model-len 28672 (up from 20480) to handle longer papers.
# Usage: sbatch scripts/sh/gemini_synthetic_reasoning/qwen3.5/blind/ailab_retry.sh

#SBATCH --job-name=qwen-blind-retry
#SBATCH --array=0-5
...
echo "=== Qwen Blind Text Review RETRY (ailab) ==="
...
nohup vllm serve "$MODEL_PATH" \
    ...
    > "logs/qwen_review/${MODE}/vllm_retry_shard${SLURM_ARRAY_TASK_ID}.log" 2>&1 &
VLLM_PID=$!
...
python scripts/qwen_batch_review.py \
    ...
    --workers $WORKERS --num_shards 6 \
    --retry_file results/reviews/blind_failed_sids.txt \
    --shard_prefix retry --wait_for_server 900 \
