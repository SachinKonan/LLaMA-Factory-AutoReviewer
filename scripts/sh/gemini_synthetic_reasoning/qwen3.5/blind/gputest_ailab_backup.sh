#!/bin/bash
# Blind text review — Qwen3.5-122B-A10B — gpu-test fallback for ailab workload (subset 0:11700)
# Usage: sbatch scripts/sh/gemini_synthetic_reasoning/qwen3.5/blind/gputest_ailab_backup.sh

#SBATCH --job-name=qwen-blind-gputest-ab
#SBATCH --array=0-18
...
echo "=== Qwen Blind Text Review (gpu-test, ailab backup) ==="
...
python scripts/qwen_batch_review.py \
    ...
    --workers $WORKERS --num_shards 19 --shard_subset "0:11700" \
    --shard_prefix ailab_gputest --wait_for_server 900 \
