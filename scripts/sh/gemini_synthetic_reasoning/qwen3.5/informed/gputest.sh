#!/bin/bash
# Informed text review — Qwen3.5-122B-A10B — gpu-test partition (shards 0-18)
# Usage: sbatch scripts/sh/gemini_synthetic_reasoning/qwen3.5/informed/gputest.sh

#SBATCH --job-name=qwen-informed-gputest
#SBATCH --array=0-18
...
#SBATCH --output=logs/qwen_review/informed/%A_%a.out
#SBATCH --error=logs/qwen_review/informed/%A_%a.err

set -euo pipefail
mkdir -p logs/qwen_review/informed results/reviews
...
echo "=== Qwen Informed Text Review (gpu-test) ==="
...
python scripts/qwen_batch_review.py \
    ...
    --workers $WORKERS --num_shards 19 --shard_subset "11700:" \
    --shard_prefix gputest --wait_for_server 900 \
