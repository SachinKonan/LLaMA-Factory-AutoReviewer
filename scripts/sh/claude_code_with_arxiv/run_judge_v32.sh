#!/bin/bash
# Run LLM-as-judge on v3.2a Claude Code reviews vs human reviews.
#
# Usage:
#   bash scripts/sh/claude_code_with_arxiv/run_judge_v32.sh
...
mkdir -p "${SCORING_DIR}/judge_scores_v32"
mkdir -p "${REPO_DIR}/logs/claude_code_with_arxiv"

N_PAPERS=$(python3 -c "import csv; print(sum(1 for _ in csv.DictReader(open('${SCORING_DIR}/judge_input_v32.csv'))))")

echo "============================================"
echo "LLM-as-Judge: v3.2a Claude Code Reviews"
echo "============================================"
...
Read judge_input_v32.csv. It has columns: submission_id, ground_truth, human_reviews_json, agent_review_json.

Create the directory judge_scores_v32/ if it does not exist.
...
- Write the scores as JSON to judge_scores_v32/{submission_id}.json in this format:
...
After all sub-agents complete, write a Python script that reads all judge_scores_v32/*.json files and merges them into judge_scores_v32_merged.json as a dict keyed by submission_id. Run the script, then print summary statistics (mean score per dimension).
...
"${CLAUDE_BIN}" -p "${JUDGE_PROMPT}" \
    ...
    --max-budget-usd 20 \
    2>&1 | tee "${REPO_DIR}/logs/claude_code_with_arxiv/judge_v32.log"
