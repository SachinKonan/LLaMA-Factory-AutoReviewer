#!/bin/bash
# Run LLM-as-judge on v3.1 agent reviews vs human reviews.
#
# Simple map-reduce: sub-agents score each paper on 5 dimensions,
# then assemble into a single JSON.
#
# Usage:
#   bash scripts/sh/claude_code_with_arxiv/run_judge_v31.sh
...
mkdir -p "${SCORING_DIR}/judge_scores_v31"
mkdir -p "${REPO_DIR}/logs/claude_code_with_arxiv"

echo "============================================"
echo "LLM-as-Judge: v3.1 Agent Reviews"
echo "============================================"

cd "${SCORING_DIR}"

read -r -d '' JUDGE_PROMPT <<'PROMPT_EOF' || true
You are an orchestrator for LLM-as-judge evaluation. Your task is to compare AI-generated paper reviews against human ICLR reviewer reviews and score alignment.

Read judge_input_v31.csv. It has columns: submission_id, ground_truth, human_reviews_json, agent_review_json.

Create the directory judge_scores_v31/ if it does not exist.
...
- Write the scores as JSON to judge_scores_v31/{submission_id}.json in this format:
...
After all sub-agents complete, write a Python script that reads all judge_scores_v31/*.json files and merges them into judge_scores_v31_merged.json as a dict keyed by submission_id. Run the script, then print summary statistics (mean score per dimension).

CRITICAL: Process ALL 129 papers. Do not skip any.
PROMPT_EOF
...
"${CLAUDE_BIN}" -p "${JUDGE_PROMPT}" \
    ...
    --max-budget-usd 20 \
    2>&1 | tee "${REPO_DIR}/logs/claude_code_with_arxiv/judge_v31.log"
