#!/bin/bash
# Claude Code agent with arxiv search via MCP.
#
# Architecture:
#   srun (background): Retrieval server on gpu-test (1 GPU, max 1hr)
#   This node:         claude -p (headless) with stdio MCP server (wraps retrieval API)
#
# Usage (from repo root):
#   bash scripts/sh/claude_code_with_arxiv/run.sh <AGENT_DIR>
#
# Examples:
#   bash scripts/sh/claude_code_with_arxiv/run.sh coding_agents_2026_codex_arxiv_aligned_with_model_v3.1
#   bash scripts/sh/claude_code_with_arxiv/run.sh coding_agents_2026_codex_arxiv_aligned_no_model_v3.1
...
echo "============================================"
echo "Claude Code Agent with Arxiv Search"
echo "============================================"
