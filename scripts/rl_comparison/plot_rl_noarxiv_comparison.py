#!/usr/bin/env python3
"""
RL NoArXiv Comparison - Compare different model architectures with GRPO training (no arXiv data).

Generates plots:
1. overall_accuracy.png - Bar chart of overall accuracy for variants with eval
...
VARIANTS = {
    "Qwen3-4B": {
        ...
        "training_col": "noarxiv_train_qwen3_4b_baseline_lowkl_3723871",
    },
    "Qwen2.5-7B": {
        "eval_pattern": "qwen2.5_7b_step55",
        "training_col": "noarxiv_qwen2.5_7b_grpo_3964390_0",
    },
    "Qwen2.5-7B-RP": {
        "eval_pattern": "qwen2.5_7b_reviewprocess",
        "training_col": "noarxiv_qwen2.5_7b_reviewprocess_4027851",
    },
    "Qwen3-8B-RP": {
        "eval_pattern": "qwen3_8b_reviewprocess",
        "training_col": "noarxiv_qwen3_8b_reviewprocess_4033553",
    },
}
...
COLORS = {
    ...
    "Qwen2.5-7B-RP": "#9467bd", # Purple
    "Qwen3-8B-RP": "#1f77b4",   # Blue
}

OUTPUT_DIR = Path("results/summarized_investigation/rl_noarxiv")
...
def load_eval_data() -> dict:
    ...
    df = pd.read_csv(DATA_DIR / "noarxiv_eval.csv")
...
def load_training_data() -> pd.DataFrame:
    ...
    df = pd.read_csv(DATA_DIR / "noarxiv_training.csv")
...
def load_entropy_data() -> pd.DataFrame:
    ...
    df = pd.read_csv(DATA_DIR / "noarxiv_entropy.csv")
...
def load_generation_length_data() -> pd.DataFrame:
    ...
    df = pd.read_csv(DATA_DIR / "noarxiv_avg_num_tokens.csv")
...
def plot_overall_accuracy(eval_data: dict) -> None:
    ...
    ax.set_title('Overall Accuracy by Model Variant (mv_combined) - NoArXiv', fontsize=14, fontweight='bold')
...
def plot_training_reward(training_data: pd.DataFrame) -> None:
    ...
    ax.set_title('Training Reward Curves (GRPO) - NoArXiv', fontsize=14, fontweight='bold')
...
def plot_training_entropy(entropy_data: pd.DataFrame) -> None:
    ...
    ax.set_title('Policy Entropy During Training - NoArXiv', fontsize=14, fontweight='bold')
    ax.legend(loc='best', fontsize=10)
    ax.grid(alpha=0.3)
    ax.set_xlim(1, 50)
...
def plot_training_generation_length(gen_length_data: pd.DataFrame) -> None:
    ...
    ax.set_title('Generation Length During Training - NoArXiv', fontsize=14, fontweight='bold')
    ax.legend(loc='best', fontsize=10)
    ax.grid(alpha=0.3)
    ax.set_xlim(1, 50)
