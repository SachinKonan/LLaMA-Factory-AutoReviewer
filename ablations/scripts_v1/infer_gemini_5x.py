...
ABLATION_V1_DATASETS = [
    ...
    # Less Critical Modifier (6 datasets)
    f"{ABLATION_V1_PREFIX}_critical_boxed_clean_test",
    f"{ABLATION_V1_PREFIX}_critical_boxed_clean_images_test",
    f"{ABLATION_V1_PREFIX}_critical_boxed_vision_test",
    f"{ABLATION_V1_PREFIX}_critical_json_clean_test",
    f"{ABLATION_V1_PREFIX}_critical_json_clean_images_test",
    f"{ABLATION_V1_PREFIX}_critical_json_vision_test",
...
# Base datasets: shared 500 samples across all modalities
BASE_DATASETS = [
    f"{ABLATION_V1_PREFIX}_base_clean_test",
    f"{ABLATION_V1_PREFIX}_base_clean_images_test",
    f"{ABLATION_V1_PREFIX}_base_vision_test",
]
...
def cmd_submit(args):
    ...
    if args.test_subset:
        ...
        if args.limit is None:
            args.limit = 10
            print(f"Test Subset mode enabled: submitting 10 samples for {len(datasets_to_submit)} datasets")
    elif args.base_only:
        datasets_to_submit = BASE_DATASETS
        print(f"Base Only mode enabled: submitting {len(datasets_to_submit)} base datasets")
    else:
...
def find_ablation_v1_job_info_files(output_dir: Path) -> list[Path]:
    ...
    # Look in each ablation dataset subdirectory (including base datasets)
    all_datasets = ABLATION_V1_DATASETS + BASE_DATASETS
...
def main():
    ...
    submit_parser.add_argument("--base_only", action="store_true",
                               help="Submit only the base datasets (3 datasets: clean, clean_images, vision)")
