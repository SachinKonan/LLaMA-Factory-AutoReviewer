...
def vllm_infer(
    ...
    # Load submission_ids from raw dataset and apply ids_file filtering + sharding
    submission_ids = None
    if ids_file is not None or num_shards > 1:
        dataset_info_path = Path(dataset_dir) / "dataset_info.json"
        with open(dataset_info_path) as f:
            dataset_info = json.load(f)

        raw_file = dataset_info[dataset]["file_name"]
        raw_path = Path(dataset_dir) / raw_file
        # If file_name points to a directory (HuggingFace-style), look for data.json inside
        if raw_path.is_dir():
            raw_path = raw_path / "data.json"
        print(f"Loading raw dataset for submission_ids: {raw_path}")
        with open(raw_path) as f:
            raw_data = json.load(f)

        # Filter by ids_file if provided
        if ids_file is not None:
            with open(ids_file) as f:
                target_ids = set(line.strip() for line in f if line.strip())
            target_indices = [
                i for i, d in enumerate(raw_data)
                if d.get("_metadata", {}).get("submission_id") in target_ids
            ]
            print(f"  Filtered by ids_file: {len(raw_data)} -> {len(target_indices)} samples")
        else:
            target_indices = list(range(len(raw_data)))

        # Apply sharding: every num_shards-th sample starting at shard_id
        shard_indices = target_indices[shard_id::num_shards]
        print(f"  Shard {shard_id}/{num_shards}: {len(shard_indices)} samples")

        submission_ids = [
            raw_data[i].get("_metadata", {}).get("submission_id", f"sample_{i:05d}")
            for i in shard_indices
        ]

        # Select matching samples from train_dataset
        train_dataset = train_dataset.select(shard_indices)
    ...
    with open(save_name, "w", encoding="utf-8") as f:
        for idx, (text, pred, label) in enumerate(zip(all_prompts, all_preds, all_labels)):
            ...
            if submission_ids is not None and idx < len(submission_ids):
                entry["submission_id"] = submission_ids[idx]
