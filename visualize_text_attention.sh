#!/bin/bash
# visualize_text_attention.sh
# Visualizes the distribution of unnormalized attention weights for each section type generated by the text model.
...
base_dir = '/scratch/gpfs/ZHUANGL/jl0796/LLaMA-Factory-AutoReviewer/outputs/best_2025_2026_text'
json_files = glob.glob(os.path.join(base_dir, '*', 'summary', 'section_attn.json'))
...
if not data_records:
    print('No data found to plot. Check if the directories contain summary/section_attn.json files.')
    exit(1)
...
plt.title('Distribution of Unnormalized Attention Weights across Section Categories (Text Model)')
plt.ylabel('Unnormalized Attention Weight')
plt.tight_layout()

out_path = os.path.join(base_dir, 'text_section_weights_boxplot.png')
