"""Utility functions for textgrad prompt optimization (v2 with prefix splitting)."""
...
# Initial human prefix from the dataset - this will be optimized
INITIAL_HUMAN_PREFIX = """I am giving you a paper. I want to predict its acceptance outcome at ICLR.
...
def load_dataset(data_dir: str, dataset_name: str) -> list[dict]:
    """
    Load ShareGPT-format dataset with prefix splitting.

    Returns list of dicts with keys:
    - system_prompt: str
    - human_prefix: str (the task instruction prefix, optimizable)
    - paper_content: str (the actual paper content)
    ...
    for item in raw_data:
        ...
        # Split human_message into prefix and paper_content
        # The paper content starts after "\n\n# " (title marker)
        parts = human_message.split("\n\n# ", 1)
        if len(parts) == 2:
            human_prefix = parts[0]
            paper_content = "# " + parts[1]
        else:
            # Fallback if split doesn't work
            human_prefix = INITIAL_HUMAN_PREFIX
            paper_content = human_message
...
def format_loss_feedback(
    ...
    Prioritizes format compliance (num_valid) when below threshold,
    then focuses on accuracy once format is reliable.
    """
    ...
    parse_failures = num_total - num_valid

    # Phase 1: Prioritize format compliance if too many parse failures
    if valid_rate < valid_threshold:
        return f"""CRITICAL: Output Format Compliance Issue

Format Compliance: {valid_rate:.1%} ({num_valid}/{num_total} predictions successfully parsed)
Parse Failures: {parse_failures} responses did not contain \\boxed{{Accept}} or \\boxed{{Reject}}
...
The human prefix instruction MUST clearly emphasize this output format requirement.
The system prompt should reinforce that the model must always provide a final verdict in the boxed format.

Current Accuracy (on valid outputs only): {metrics['accuracy']:.1%}

DO NOT focus on improving accuracy until format compliance reaches at least {valid_threshold:.0%}.
First ensure all responses contain the required \\boxed{{}} format."""

    # Phase 2: Once format is good, focus on accuracy
    return f"""Evaluation Results for Paper Acceptance Prediction:
- Format Compliance: {valid_rate:.1%} ({num_valid}/{num_total} predictions parsed) ✓
...
Both the system prompt (reviewer role/approach) and human prefix (task instruction)
should be optimized to guide the model to be a better academic reviewer."""
